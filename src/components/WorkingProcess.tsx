    ...
    <div className="p-4 pl-6 lg:pl-20 lg:p-10">
      ...
      <div className="p-4 pl-6 lg:pl-20 lg:p-10">
        {processkeys.map((val, idx) => (
          <div
            key={val.key}
            className={`workingP autoShow flex flex-row md:flex-row w-[392px] md:w-full p-6 justify-between items-center border-1 border-[#191A23] mb-6 ${expandedItems[val.key] ? 'bg-[#B9FF66]' : 'bg-[#F3F3F3]'}`}
          >
            <div key={idx} className='w-full'>
              <div className='flex flex-row items-center justify-between gap-6' onClick={() => toggleItem(val.key)}>
                <h1 className='num lg:text-6xl lg:font-medium'>{val.num}</h1>
                <h3 className="text-xl font-bold lg:text-3xl lg:font-medium">{val.title}</h3>
                <Image src={expandedItems[val.key] ? val.expandedIcon : val.icon} alt="icon" width={30} height={30} className='cursor-pointer lg:size-[58px]' />
              </div>

              {expandedItems[val.key] && (
                <>
                  <div className='border-b border-black md:w-full p-6'></div>
                  <p className="text-base mt-2 lg:text-lg lg:font-normal">{val.description}</p>
                </>
              )}
            </div>
          </div>
